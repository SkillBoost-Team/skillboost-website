...
<body>
    ...
    <div class="voice-input-wrapper">
        ...
        <button type="button" class="voice-btn" id="voiceBtn" title="<?= $lang === 'en' ? 'Voice input' : ($lang === 'es' ? 'Entrada de voz' : ($lang === 'ar' ? 'إدخال صوتي' : 'Saisie vocale')) ?>">
                    ...
                    <div class="bg-light p-4 rounded-3 mb-4">
                        ...
                        <form method="GET" action="" class="mb-4">
                            <div class="input-group">
                                <input type="number" class="form-control" name="id" placeholder="<?= $t['search_placeholder'] ?>" 
                        ...
                        <?php if ($reclamation_data): ?>
                            <div class="reponse-container">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <h4><?= $t['claim_number'] ?><?= $reclamation_data['reclamation']['id'] ?></h4>
                                    <a href="https://wa.me/21690044054?text=<?= urlencode($lang === 'en' ? 'Hello, I\'m contacting about my complaint #' : ($lang === 'es' ? 'Hola, me contacto sobre mi reclamación #' : ($lang === 'ar' ? 'مرحبًا ، أنا أتصل بشأن شكواي رقم #' : 'Bonjour, je contacte au sujet de ma réclamation #'))) . $reclamation_data['reclamation']['id'] ?>"
                                ...
                                <div class="card mb-3">
                                    <div class="card-header bg-primary text-white">
                                        <strong><?= $t['claim_details'] ?></strong>
                                    </div>
                                    <div class="card-body">
                                        <p><strong><?= $lang === 'en' ? 'Full Name:' : ($lang === 'es' ? 'Nombre Completo:' : ($lang === 'ar' ? 'الاسم الكامل:' : 'Nom Complet:')) ?></strong> <?= htmlspecialchars($reclamation_data['reclamation']['full_name']) ?></p>
                                        <p><strong><?= $lang === 'en' ? 'Email:' : ($lang === 'es' ? 'Correo:' : ($lang === 'ar' ? 'البريد الإلكتروني:' : 'Email:')) ?></strong> <?= htmlspecialchars($reclamation_data['reclamation']['email']) ?></p>
                                        <p><strong><?= $lang === 'en' ? 'Subject:' : ($lang === 'es' ? 'Asunto:' : ($lang === 'ar' ? 'الموضوع:' : 'Sujet:')) ?></strong> 
<?= isset($reclamation_data['reclamation']['subject']) ? htmlspecialchars($reclamation_data['reclamation']['subject']) : 'Non spécifié' ?></p>

<p><strong><?= $lang === 'en' ? 'Type:' : ($lang === 'es' ? 'Tipo:' : ($lang === 'ar' ? 'النوع:' : 'Type:')) ?></strong> 
<?= isset($reclamation_data['reclamation']['type']) ? getTypeText($reclamation_data['reclamation']['type']) : 'Non spécifié' ?></p>

<p><strong><?= $t['status'] ?>:</strong> 
<span class="<?= isset($reclamation_data['reclamation']['status']) ? getStatusClass($reclamation_data['reclamation']['status']) : 'badge bg-secondary' ?>">
<?= isset($reclamation_data['reclamation']['status']) ? getStatusText($reclamation_data['reclamation']['status']) : 'Inconnu' ?>
</span></p>
                                    </div>
                                </div>
...
if ('webkitSpeechRecognition' in window) {
    ...
    recognition.lang = '<?= $lang ?>-<?= strtoupper($lang) ?>';
    ...
    recognition.onresult = (event) => {
        ...
        // Ajoute le texte reconnu à la position actuelle du curseur
    ...
    recognition.onerror = (event) => {
        ...
        const errorMsg = '<?= $lang === 'en' ? 'Voice recognition error' : ($lang === 'es' ? 'Error de reconocimiento' : ($lang === 'ar' ? 'خطأ في التعرف على الصوت' : 'Erreur de reconnaissance')) ?>';
        alert(errorMsg);
    };
} else {
    // Cache le bouton si la reconnaissance vocale n'est pas supportée
    voiceBtn.style.display = 'none';
    
    // Option: Afficher un message que la fonctionnalité n'est pas disponible
